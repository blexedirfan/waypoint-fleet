...
notificationsRouter.post("/", requireAuth, (req, res) => {
  ...
  db.prepare("INSERT INTO notifications (id, title, desc, tone, read, created_at) VALUES (?, ?, ?, ?, ?, ?)").run(
...
notificationsRouter.patch("/:id/toggle", requireAuth, (req, res) => {
  const row = db.prepare("SELECT * FROM notifications WHERE id = ?").get(req.params.id);
  if (!row) return res.status(404).json({ error: "Notification not found." });
  db.prepare("UPDATE notifications SET read = ? WHERE id = ?").run(row.read ? 0 : 1, row.id);
  const rows = db.prepare("SELECT * FROM notifications ORDER BY created_at DESC").all();
  res.json(rows.map(rowToNotification));
});
